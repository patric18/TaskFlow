...
test.describe('Onboarding', () => {
  test('new user completes full onboarding flow', flaky, async ({ page }) => {
    ...
    await onboarding.continueButton().click();
    await expect(page.getByRole('heading', { name: /invite/i })).toBeVisible({ timeout: 15000 });
    await onboarding.skipInviteButton().click();

    await expect(page).toHaveURL(/\/(projects|dashboard)/, { timeout: 15000 });
