...
test.describe('Task detail', () => {
  ...
  test('add comment and see it immediately', async ({ page, request, e2eProjectId, ownerToken, authenticatedPage }) => {
    ...
    await expect(modal.dialog().getByText(commentText)).toBeVisible();
...
test.describe('Task detail — negative & edge cases', () => {
  ...
  test('markdown or HTML in comment is escaped, not rendered as HTML', async ({ page, e2eProjectId, authenticatedPage }) => {
    const kanban = new KanbanPage(page);
    await kanban.goto(e2eProjectId);
    ...
    await expect(modal.dialog().getByText(payload)).toBeVisible();
  ...
  test('task title cannot be saved as empty string', async ({ page, request, e2eProjectId, ownerToken, authenticatedPage }) => {
    const kanban = new KanbanPage(page);
    await kanban.goto(e2eProjectId);
  ...
  test('task title with 256 characters is rejected', async ({ page, request, e2eProjectId, ownerToken, authenticatedPage }) => {
    const kanban = new KanbanPage(page);
    await kanban.goto(e2eProjectId);
  ...
  test('rapidly clicking save on create task does not create duplicates', async ({ page, request, e2eProjectId, ownerToken, authenticatedPage }) => {
    const kanban = new KanbanPage(page);
    ...
    await expect(kanban.taskCardByTitle(title)).toHaveCount(1);
